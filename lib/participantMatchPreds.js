...
import { normalizeInicioKoPreds, lookupEliminatoriasKoPred } from './knockoutBridge.js'
import { resolveKnockoutAdvanceSide } from './knockoutAdvances.js'
import { getPredSavedAt } from './predictionTimestamps.js'
...
export function getParticipantPredsForMatch(
  ...
  const matchRow =
    (knockoutMatches || []).find(m => String(m.id) === matchKey)
    || (knockoutMatches || []).find(m => m.matchNumber != null && String(m.matchNumber) === matchKey)
    || { id: matchKey }
  ...
  for (const p of Object.values(participants || {})) {
    ...
    const koPred = lookupEliminatoriasKoPred(migrated.knockout, matchRow)
    const pred =
