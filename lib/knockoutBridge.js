...
function resolveBracketSideOrPlaceholder(ref, byMatchNum, preds) {

  return resolveBracketSide(ref, byMatchNum, preds) || placeholderBracketSide(ref)
...
function appendBracketRounds(r32Schedule, inicioKoPreds) {
  ...
  for (const slot of KNOCKOUT_BRACKET_TREE) {
    ...
    const home = resolveBracketSideOrPlaceholder(slot.home, byMatchNum, preds)

    const away = resolveBracketSideOrPlaceholder(slot.away, byMatchNum, preds)
