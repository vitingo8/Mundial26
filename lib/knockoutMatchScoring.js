...
/** Partidos canónicos de eliminatorias para puntuar (como el calendario; un slot FIFA como máximo). */
export function listEliminatoriasScoringMatches(
  knockoutMatches = [],
  koPreds = {},
  scheduleOpts = {},
) {
  const schedule = buildEliminatoriasKnockoutSchedule(knockoutMatches, koPreds, scheduleOpts)
  ...
  return schedule.filter(m => {
    ...
    if (m.matchNumber == null) return true
    if (seenNumbers.has(m.matchNumber)) return false
