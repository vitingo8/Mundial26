...
describe('generateRoundOf32', () => {
  ...
  it('generates all 16 round-of-32 matches from full bracket map', async () => {
    const { roundOf32Map } = await import('./dist/config/roundOf32Map.js')
    const { thirdPlaceCombinationMap } = await import('./dist/config/thirdPlaceCombinationMap.js')
    assert.equal(roundOf32Map.length, 16)
    const result = generateRoundOf32(buildSyntheticGroups(), roundOf32Map, thirdPlaceCombinationMap)
    assert.equal(result.matches.length, 16)
    assert.equal(result.combinationKey, 'ACDFGHIJ')
  })

  it('resolves real FIFA combination CEGHIJKL from full map', async () => {
    const { thirdPlaceCombinationMap } = await import('./dist/config/thirdPlaceCombinationMap.js')
    assert.ok(thirdPlaceCombinationMap.CEGHIJKL)
    assert.equal(thirdPlaceCombinationMap.CEGHIJKL[74], 'C')
    assert.equal(thirdPlaceCombinationMap.CEGHIJKL[77], 'G')
    assert.equal(thirdPlaceCombinationMap.CEGHIJKL[79], 'E')
  })
