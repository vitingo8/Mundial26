...
import { buildMergedResults, resultsNeedSync, buildResultTimestamps } from './syncResultsFromApi.js'
import {
...
export async function syncAllGroupsFromApi(options = {}) {
  ...
  for (const group of groups || []) {
    ...
    if (!resultsNeedSync(group.results, merged)) continue
    const resultsUpdatedAt = buildResultTimestamps(group.results, group.results_updated_at, merged)
    const { error } = await supabase
      .from('porra_groups')
      .update({ results: merged, results_updated_at: resultsUpdatedAt })
      .eq('id', group.id)
