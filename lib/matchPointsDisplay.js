import { calcMatchPointsSplit, SCORING, PHASE_WEIGHT } from './gameData.js'
import { migratePredictionMap } from './matchIdMap.js'
import { knockoutRealKoMatchId, inicioKoMatchId } from './knockoutBridge.js'
import { resolveKnockoutResult } from './knockoutMatchScoring.js'

function roundRankingPts(n) {
  return Math.round(n * 10) / 10
}

function appendRankingDetail(detail, rankingPts, label) {
  if (!rankingPts || rankingPts <= 0) return detail
  const suffix = ` → ${rankingPts} pts al ranking (${label})`
  return detail === 'Sin puntos' ? suffix.trim() : `${detail}${suffix}`
}

/**
 * Resumen de puntos de un partido (predicción vs resultado publicado).
 * @returns {{ pts: number, rankingPts: number, detail: string, parts: string[] } | null}
 */
export function summarizeMatchPoints(prediction, result, opts = {}) {
  ...
  const rankingWeight = opts.knockout ? PHASE_WEIGHT.knockoutReal : 1
  const rankingPts = roundRankingPts(pts * rankingWeight)
  const baseDetail = pts === 0 ? 'Sin puntos' : parts.join(' · ')
  const rankingLabel = opts.knockout ? 'Eliminatorias ×40%' : null
  return {
    pts,
    rankingPts,
    detail: rankingLabel ? appendRankingDetail(baseDetail, rankingPts, rankingLabel) : baseDetail,
    parts,
  }
}
...
export function buildPublishedResultsMap(results, phase = 'group', canonicalMatches = [], resultsUpdatedAt = null) {
  ...
  const inicioFromReal = addInicioKoResultAliases(kFinal)
  return { ...gFinal, ...inicioFromReal, ...inicio }
}

/** Copia resultados reales (id numérico) a inicio-ko-N para el cuadro previsto. */
function addInicioKoResultAliases(map = {}) {
  const out = {}
  for (const [id, result] of Object.entries(map || {})) {
    if (isInicioKoId(id)) continue
    if (result?.home == null || result?.away == null) continue
    const num = result.matchNumber
    if (num == null) continue
    const inicioId = inicioKoMatchId(num)
    if (map[inicioId] == null && out[inicioId] == null) out[inicioId] = result
  }
  return out
}
