...
describe('buildEliminatoriasKnockoutSchedule', () => {
  ...
  it('uses API id for r32 and knockout-ko for later rounds', () => {
    ...
    assert.equal(m89.id, knockoutRealKoMatchId(89))
    assert.equal(m89.isPredictedBracket, true)
  })

  it('octavos teams follow user picks in dieciseisavos', () => {
    const preds = {
      'api-74': { home: 2, away: 0 },
      'api-77': { home: 1, away: 0 },
    }
    const api = [
    ...
    const schedule = buildEliminatoriasKnockoutSchedule(api, preds)
    const m89 = schedule.find(m => m.matchNumber === 89)
    assert.ok(m89)
