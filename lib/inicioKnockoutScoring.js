...
import { getApiMatchDisplayScore, indexApiMatches } from './apiMatchScores.js'
...
export function buildInicioKnockoutScoringState(
  ...
  const groupPreds = groupMatches.length
    ? migratePredictionMap(preds.group || {}, groupMatches).migrated
    : (preds.group || {})
  ...
  for (const m of knockoutMatches || []) {
    ...
    const apiRow = apiById[resId] || m
    let published = enrichPublishedResult(publishedRaw, {
      id: resId,
      matchNumber: m.matchNumber,
      apiRow,
    })
    if (!published && apiRow?.status === 'FINISHED') {
      const apiScore = getApiMatchDisplayScore(apiRow)
      if (apiScore) {
        published = enrichPublishedResult(apiScore, {
          id: resId,
          matchNumber: m.matchNumber,
          apiRow,
        })
...
export function calcInicioKnockoutPointsSplit(pred, predictedTeams, state, matchNumber) {
  ...
  let real = findRealKnockoutMatchForPair(
    predictedTeams.home,
    predictedTeams.away,
    state,
    num,
  ...
  if (!real && num != null) {
    const slotEntry = findRegisteredMatchByNumber(state, num)
    if (
      slotEntry?.result?.home != null &&
      slotEntry?.teams &&
      teamsPairMatches(
        predictedTeams.home,
        predictedTeams.away,
        slotEntry.teams.home,
        slotEntry.teams.away,
      )
    ) {
      real = slotEntry
    }
  ...
  if (
    !pairingEliminated &&
    realResult?.home != null &&
    realResult?.away != null &&
    realTeams
  ) {
...
export function summarizeInicioKnockoutMatchPoints(pred, predictedTeams, state, matchNumber) {
  ...
  const rankingPts = Math.round(pts * PHASE_WEIGHT.inicio * 10) / 10
  const baseDetail = pts === 0 ? 'Sin puntos' : parts.join(' · ')
  const detail =
    rankingPts > 0
      ? (baseDetail === 'Sin puntos'
        ? `${rankingPts} pts al ranking (Inicio ×60%)`
        : `${baseDetail} → ${rankingPts} pts al ranking (Inicio ×60%)`)
      : baseDetail
  return {
    pts,
    rankingPts,
...
/** Burbuja «0 pts» solo si el cruce es void y no hay ningún punto (p. ej. +1 pasa). */
export function shouldShowInicioKnockoutVoidZero(uiStatus, summary) {
  return uiStatus?.void === true && !(summary?.pts > 0)
}
