...
export function buildInicioKnockoutScoringState(
  ...
  for (const [id, teams] of Object.entries(inicioPredictedById)) {
    const res = knockoutResults?.[id]
    if (res?.home == null || res?.away == null) continue
    if (!isResolvedTeamName(teams.home) || !isResolvedTeamName(teams.away)) continue
    registerKnockoutMatch(state, teams.home, teams.away, {
      id,
      matchNumber: teams.matchNumber,
      roundId: teams.roundId ?? knockoutRoundIdFromMatchNumber(teams.matchNumber),
      result: res,
