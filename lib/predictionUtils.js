  ...
  function addMatch(m, pred, res, phase) {
    ...
    items.push({
      ...
      detail: raw === 0
        ? 'Sin puntos'
        : exact
          ? `+${SCORING.correctOutcome} (1X2) +${SCORING.exactScore} exacto = ${raw} pts`
          : outcome
            ? `+${SCORING.correctOutcome} (1X2)`
            : '',
    })
  }

  groupMatches.forEach(m => addMatch(m, preds.group?.[m.id], results.group?.[m.id], 'group'))
  knockoutMatches.forEach(m => addMatch(m, preds.knockout?.[m.id], results.knockout?.[m.id], 'knockout'))
...
export function getAdminTaskBadges(group) {
  const badges = []
  if (!group.group_deadline && !group.knockout_deadline) {
    badges.push({ type: 'warn', text: 'Configura los plazos en Organización' })
  }
