...
export function buildPointsBreakdown(user, group, groupMatches = [], knockoutMatches = []) {
  ...
  const inicioKoState = buildInicioKnockoutScoringState(user, {
    groupMatches,
    knockoutMatches,
    knockoutResults: results.knockout,
    groupResults: results.group,
  })
  const elimSchedule = listEliminatoriasScoringMatches(knockoutMatches, preds.knockout, {
    groupMatches,
    apiMatches: group?.apiMatches,
  })
  const items = []
  ...
  Object.entries(preds.inicioKnockout || {}).forEach(([id, pred]) => {
    if (!isInicioKoId(id) || !pred) return
    const split = calcInicioKnockoutPointsForId(id, pred, inicioKoState)
    const raw = split.gep + split.resultado + split.advance
    const w = phasePts(raw, 'inicio')
    const parts = []
    ...
    items.push({
      id,
      phase: 'inicio',
      label: `KO previsto ${id.replace(/^inicio-ko-/, 'P')}`,
      pred: `${pred.home}-${pred.away}${pred.advances ? ` (${pred.advances} pasa)` : ''}`,
      real: '—',
      pts: raw,
      weightedPts: w.weighted,
      factor: w.weight,
  ...
  const bonusFields = [
    ...
    { id: 'topKeeper', label: 'Mejor portero' },
  ...
  bonusFields.forEach(f => {
    ...
    const hit = p && a && bonusPlayerNamesMatch(p, a)
    if (p && a) {
      items.push({
        id: f.id,
        phase: 'bonus',
...
export function getAdminTaskBadges(group) {
  ...
  if (!group.knockout_deadline) {
    badges.push({ type: 'info', text: 'Eliminatorias: por partido hasta el pitido' })
  }
