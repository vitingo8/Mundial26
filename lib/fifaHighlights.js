...
/** Slugs FIFA distintos del nombre canónico (ver teamsModule API / página de equipos). */
const FIFA_SLUG_OVERRIDES = {
  'united states': 'usa',
...
/** Slug FIFA para URLs de resumen: usa, korea-republic, south-africa, etc. */
export function fifaTeamSlug(teamName) {
  const canonical = toCanonicalTeamName(teamName)
  if (!canonical) return ''
  const base = FIFA_SLUG_OVERRIDES[canonical] || canonical
  return base
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]+/g, '-')
