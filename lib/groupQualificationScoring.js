...
/**
 * Clasificados reales: FotMob → R32 API → tablas desde resultados publicados en la porra.
 * Durante el torneo, con API en vivo y grupos abiertos, no se usa R32 parcial (espera FotMob).
 */
export function resolveActualQualifiers({
  fotmobStandings,
  knockoutMatches,
  groupMatches,
  apiMatches = [],
  ...
  const allGroupsComplete = completed.size >= GROUP_LETTERS.length
  ...
  const fromR32 = buildActualQualifiersFromApiR32(knockoutMatches, groupMatches, completed)
  if (fromR32.ready) return fromR32

  return buildActualByGroupFromStandings(groupMatches, groupResults, completed)
}
