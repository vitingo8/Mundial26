...
import { mergePredictions, summarizePredictions, countMergeableGaps, pushPredictionsToParticipant } from './predictionMirror.js'
...
describe('predictionMirror', () => {
  ...
  it('countMergeableGaps counts only empty slots in target', () => {
    const target = {
      group: { m1: { home: 1, away: 0 } },
      knockout: {},
      inicioKnockout: {},
    ...
    assert.equal(countMergeableGaps(target, source), 3)
  })

  it('pushPredictionsToParticipant merges before save', async () => {
    let saved = null
    const supabase = {
      from() {
        return {
    ...
    const merged = await pushPredictionsToParticipant(
      supabase,
      'p2',
      { group: { m1: { home: 1, away: 0 } } },
      { group: { m2: { home: 2, away: 2 } } },
      'fillGaps',
    )
    assert.deepEqual(merged.group.m1, { home: 1, away: 0 })
    assert.deepEqual(merged.group.m2, { home: 2, away: 2 })
    assert.ok(saved?.predictions)
  })
})
