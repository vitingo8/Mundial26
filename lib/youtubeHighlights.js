...
/** Canales preferidos (se prueban primero si hay varios candidatos). */
const PREFERRED_CHANNELS = [
  'fifa', 'teledeporte', 'rtve', 'dazn', 'telemundo', 'tudn', 'vix', 'gol mundial',
]
...
async function searchYoutube(query) {
  ...
  const res = await fetch(url, {
    headers: { 'User-Agent': SEARCH_UA, 'Accept-Language': 'es-ES,es;q=0.9,en;q=0.8' },
  })
  if (!res.ok) return []
...
export async function findEmbeddableHighlights(homeTeam, awayTeam) {
  ...
  const queries = [
    ...
    `${homeTeam} ${awayTeam} highlights FIFA World Cup 2026`,
  ]
