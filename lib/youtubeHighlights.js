...
import staticPlaylistCache from './daznPlaylistCache.json' with { type: 'json' }
...
async function fetchDaznPlaylistFromWeb() {
  ...
  const fromHtml = data ? extractPlaylistVideosFromNode(data).filter(isDaznEntry) : []
  if (fromHtml.length >= 20) return fromHtml
  const fromInnertube = await fetchDaznPlaylistViaInnertube(html)
  return dedupeEntries([...fromHtml, ...fromInnertube])
}

async function fetchDaznPlaylistViaInnertube(html) {
      ...
      body: JSON.stringify({
        context: {
          client: {
            ...
            gl: 'ES',
            clientName: 'WEB',
            clientVersion: clientVersion || '2.20260625.01.00',
          },
        },
        browseId: `VL${DAZN_PLAYLIST_ID}`,
      }),
    })
    if (!res.ok) return []
    return extractPlaylistVideosFromNode(await res.json()).filter(isDaznEntry)
  } catch {
    return []
  }
}

function staticCacheEntries() {
  return (staticPlaylistCache?.entries || []).filter(isDaznEntry)
}
...
async function getDaznPlaylistEntries() {
  ...
  try {
    ...
    const entries = dedupeEntries([...webEntries, ...rssEntries, ...staticCacheEntries()])
    if (entries.length) {
