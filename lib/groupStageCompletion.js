...
/**
 * Plazas para el cuadro KO: 1.º/2.º confirmados aunque el grupo siga abierto;
 * 3.º solo cuando el grupo está cerrado (los mejores terceros van aparte).
 */
export function buildBracketQualifiersFromFotmob(byGroup = {}, completedGroups) {
  const completed = completedGroups instanceof Set ? completedGroups : new Set(completedGroups || [])
  const out = emptyGroupMap()
  for (const letter of GROUP_LETTERS) {
    const row = byGroup[letter] || {}
    ...
    out[letter] = { 1: row[1] ?? null, 2: row[2] ?? null, 3: null }
  }
  return out
}
