...
test('migratePredictionMap reindexa catalog-N al calendario FotMob', async () => {
  const { getWcMatchesSafe } = await import('./fotmobServerCache.js')
  ...
  const { migrated, orphans, moved } = migratePredictionMap(
    { 'catalog-6': { home: 1, away: 2 } },
    groupMatches,
  )

  assert.equal(orphans.length, 0)
  assert.equal(moved, 1)
  assert.equal(migrated['catalog-6'], undefined)
  assert.ok(migrated['4667772'])
})
