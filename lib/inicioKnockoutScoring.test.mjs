...
describe('inicioKnockoutScoring', () => {
  ...
  it('advance +1 when predicted opponent was eliminated earlier in the bracket', () => {
    const state = {
      pairIndexByRound: {
        r32: {
          [knockoutPairKey('Sudáfrica', 'Canadá')]: {
            teams: { home: 'Sudáfrica', away: 'Canadá' },
            matchNumber: 73,
            roundId: 'r32',
        ...
        r16: {
          [knockoutPairKey('Canadá', 'Marruecos')]: {
            teams: { home: 'Canadá', away: 'Marruecos' },
            matchNumber: 89,
            roundId: 'r16',
      ...
      actualByMatchNumber: {
        73: { home: 'Sudáfrica', away: 'Canadá', roundId: 'r32' },
        89: { home: 'Canadá', away: 'Marruecos', roundId: 'r16' },
      },
      eliminatedTeams: new Map([[normalizeTeamName('Sudáfrica'), 'r32']]),
    }

    const split = calcInicioKnockoutPointsSplit(
      { home: 0, away: 3 },
      { home: 'Sudáfrica', away: 'Marruecos', matchNumber: 90, roundId: 'r16' },
      state,
      90,
    )
    assert.equal(split.gep, 0)
  ...
  it('does not attach published result to predicted teams when real homeTeam/awayTeam differ', () => {
    const state = buildInicioKnockoutScoringState(
      {
        predictions: {
          group: {},
          inicioKnockout: {
            [inicioKoMatchId(73)]: { home: 1, away: 2 },
            [inicioKoMatchId(76)]: { home: 3, away: 1 },
          },
        },
      },
      {
        groupMatches: [],
        knockoutMatches: [
          { id: 'api-73', matchNumber: 73, roundId: 'r32', home: 'Sudáfrica', away: 'Canadá' },
          { id: 'api-76', matchNumber: 76, roundId: 'r32', home: 'Brasil', away: 'Japón' },
        ],
        knockoutResults: {
          [inicioKoMatchId(73)]: {
            home: 0,
    ...
    state.inicioPredictedById[inicioKoMatchId(76)] = {
      home: 'Brasil',
      away: 'Suecia',
      matchNumber: 76,
      roundId: 'r32',
    ...
    assert.equal(state.actualByMatchNumber[73]?.home, 'Sudáfrica')
    assert.equal(state.actualByMatchNumber[73]?.away, 'Canadá')
    assert.equal(state.actualByMatchNumber[76]?.home, 'Brasil')
    assert.equal(state.actualByMatchNumber[76]?.away, 'Japón')

    assert.equal(
      findRealKnockoutMatchForPair('República Checa', 'Suiza', state, 73),
      null,
    )
    assert.equal(
      findRealKnockoutMatchForPair('Brasil', 'Suecia', state, 76),
      null,
    )

    assert.equal(getInicioKnockoutUiStatus('República Checa', 'Suiza', 73, state).void, true)
    assert.equal(getInicioKnockoutUiStatus('Brasil', 'Suecia', 76, state).void, true)

    const p73 = calcInicioKnockoutPointsSplit(
      { home: 1, away: 2 },
      { home: 'República Checa', away: 'Suiza' },
      state,
      73,
    )
    const p76 = calcInicioKnockoutPointsSplit(
      { home: 3, away: 1 },
      { home: 'Brasil', away: 'Suecia' },
      state,
      76,
    )
    assert.equal(p73.gep + p73.resultado + p73.advance, 0)
    assert.equal(p76.gep, 0)
    assert.equal(p76.resultado, 0)
    assert.equal(p76.advance, SCORING.knockoutAdvance)
  })

  it('void UI may still show +1 pasa when pairing is void', () => {
    const state = {
      pairIndexByRound: {
        r32: {
          [knockoutPairKey('Sudáfrica', 'Canadá')]: {
    ...
    const teams = { home: 'Sudáfrica', away: 'Marruecos' }
    const ui = getInicioKnockoutUiStatus(teams.home, teams.away, 90, state)
    const summary = summarizeInicioKnockoutMatchPoints(
      { home: 0, away: 3 },
      teams,
      state,
      90,
    )
    assert.equal(ui.void, true)
    assert.equal(summary.pts, SCORING.knockoutAdvance)
    assert.equal(shouldShowInicioKnockoutVoidZero(ui, summary), false)
  })
