...
function isConfirmedQualifier(row) {
  return row?.qualColor === FOTMOB_QUALIFIED_COLOR
}
...
export function transformFotmobStandings(apiResponse, groupMatches = []) {
  ...
  for (const block of tables) {
    ...
    if (first?.name && isConfirmedQualifier(first)) byGroup[letter][1] = first.name
    if (second?.name && isConfirmedQualifier(second)) byGroup[letter][2] = second.name
  }
