/**
 * Eliminatorias: empate en el marcador → elegir quién pasa; +1 pt si aciertas el clasificado.
 */
...
/**
 * Quién pasa según marcador 90' o penaltis en datos API/FotMob.
 * @returns {'home'|'away'|null}
 */
export function inferKnockoutAdvancesFromApiMatch(apiMatch) {
  ...
  const fromScore = apiMatch.score.penaltyShootoutWinner
...
/** Añade `advances` al resultado publicado si hubo penaltis o ganador en API. */
export function enrichKnockoutResultWithAdvances(result, apiMatch) {
  ...
  if (result.advances === 'home' || result.advances === 'away') return result
  const advances = inferKnockoutAdvancesFromApiMatch(apiMatch)
  if (!advances) return result
  if (!isKnockoutScoreDraw(result)) return result
