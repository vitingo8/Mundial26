...
import { fetchFotmobGroupStandings } from './fotmobStandings.js'
...
const DETAIL_PAGE_TTL_LIVE_MS = 8_000
...
export async function enrichFinishedKnockoutMatches(matches) {
  const finished = (matches || []).filter(m =>
    m.stage && m.stage !== 'GROUP_STAGE' && m.status === 'FINISHED',
  )
  if (!finished.length) return matches

  await Promise.all(finished.map(async m => {
    try {
      const detail = await getFotmobMatchDetail(String(m.id))
      if (detail.rawEvents?.length) m.rawEvents = detail.rawEvents
      const reg = getRegulationTimeScore(detail)
      if (reg) {
        m.score = { ...m.score, regulationTime: reg }
      }
      const pen = detail.score?.penaltyShootoutWinner
        ?? (detail.score?.winner === 'DRAW' ? inferKnockoutAdvancesFromApiMatch(detail) : null)
      if (pen) {
        m.score = { ...m.score, penaltyShootoutWinner: pen }
      }
    } catch {
      // sin detalle se usa fullTime si no hubo prórroga
    }
  }))
  return matches
}
...
export async function fetchWcMatchesCached() {
  ...
  try {
    ...
    // Penaltis y rawEvents antes de numerar: sin el ganador real, el cuadro no
    // resuelve octavos (p. ej. 89 Paraguay–Francia, 93 Portugal–España).
    await enrichFinishedKnockoutMatches(normalized)

    const enrichedMatches = enrichApiMatches(normalized)
    await enrichFinishedKnockoutMatches(enrichedMatches)
    let standings = null
    try {
      standings = await fetchFotmobGroupStandings(enrichedMatches)
    } catch {
      standings = matchesCache.data?.standings ?? null
    }
...
/** @type {Map<string, { players: object; template: string; ts: number }>} */
const heatmapCache = new Map()

/** SVG del heatmap de un jugador (FotMob). Las claves usan optaId, no el id de FotMob. */
export async function getFotmobPlayerHeatmap(matchId, playerId, options = {}) {
  const key = String(matchId)
  const pid = String(playerId)
  let heatmapPlayerId = options.optaId != null ? String(options.optaId) : null
  let pubUrl = options.heatmapPubUrl || null

  if (!heatmapPlayerId || !pubUrl) {
    try {
      const detail = await getFotmobMatchDetail(key)
      if (!heatmapPlayerId) {
        const optaId = detail?.playerStatsById?.[pid]?.optaId
        if (optaId != null) heatmapPlayerId = String(optaId)
      }
      pubUrl = pubUrl || detail?.heatmapPubUrl || null
    } catch {
  ...
  if (!players || !template) {
    ...
    try {
      ...
      if (players && template) heatmapCache.set(key, { players, template, ts: Date.now() })
    } catch {
      return null
    }
  }

  if (!players || !template) return null

  const lookupKeys = [
    heatmapPlayerId && `p${heatmapPlayerId}`,
    heatmapPlayerId,
    `p${pid}`,
    pid,
  ].filter(Boolean)
  ...
  for (const lookupKey of lookupKeys) {
    if (players[lookupKey]) {
      return { circles: players[lookupKey], template }
    }
  }
  return null
}
