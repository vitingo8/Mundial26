...
async function attachPenaltyWinnersToKnockoutDraws(matches) {
  const draws = (matches || []).filter(m =>
    m.stage && m.stage !== 'GROUP_STAGE'
    && m.status === 'FINISHED'
    && m.score?.winner === 'DRAW'
    && !m.score?.penaltyShootoutWinner,
  )
  if (!draws.length) return matches

  await Promise.all(draws.map(async m => {
    try {
      const detail = await getFotmobMatchDetail(String(m.id))
      const side = inferKnockoutAdvancesFromApiMatch(detail)
      if (side) {
        m.score = { ...m.score, penaltyShootoutWinner: side }
      }
    } catch {
      // sin detalle no se puede inferir la tanda
    }
  }))
  return matches
}
...
export async function fetchWcMatchesCached() {
  ...
  try {
    ...
    await attachPenaltyWinnersToKnockoutDraws(enrichedMatches)
    let standings = null
    try {
