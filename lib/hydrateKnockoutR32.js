...
function isDefinitiveThirdGroup(group, allByGroup, bestThirds, completed) {
  if (!completed.has(group)) return false
  const team = allByGroup[group]?.[3]
  if (!team) return false
  const row = (bestThirds || []).find(t => teamsMatch(t.name, team))
  return Boolean(row?.qualifies)
}
...
function resolveSlotTeamName(
  ...
  if (parsed.position === 3) {
    ...
    if (!isDefinitiveThirdGroup(assigned, allByGroup, bestThirds, completed)) return null
 ...
 * Terceros: solo nombre si el grupo está cerrado y FotMob confirma el mejor tercero (verde).
 */
