...
/** Asignaciones 3.º → partido según cruces API ya resueltos. */
...
function resolveSlotTeamName(source, byGroup, completed, thirdAssignments, matchNumber) {
  const parsed = parseGroupSlotSource(source)
  if (!parsed) return null

  if (parsed.position === 3) {
    const assigned = thirdAssignments[matchNumber]
    if (assigned && completed.has(assigned) && byGroup[assigned]?.[3]) {
      return byGroup[assigned][3]
    }
    const usedGroups = new Set(Object.values(thirdAssignments))
    const candidates = parsed.groups.filter(
      g => completed.has(g) && byGroup[g]?.[3] && !usedGroups.has(g),
    )
    if (candidates.length === 1) return byGroup[candidates[0]][3]
    return null
  }

  const group = parsed.groups[0]
  if (!completed.has(group)) return null
  return byGroup[group]?.[parsed.position] ?? null
}

function resolveBracketSide(source, apiTeamName, byGroup, completed, thirdAssignments, matchNumber, teamToGroup) {
  const parsed = parseGroupSlotSource(source)
  ...
  if (parsed?.position === 3) {
    if (isResolvedTeamName(apiTeamName)) {
      const group = resolveTeamGroup(apiTeamName, teamToGroup)
      if (
        group &&
        parsed.groups.includes(group) &&
        byGroup[group]?.[3] &&
        teamsMatch(byGroup[group][3], apiTeamName)
      ) {
        return apiTeamName
      }
    }
 ...
 * Prioriza standings sobre etiquetas FIFA crudas de la API.
 */
export function hydrateKnockoutR32FromStandings(
  ...
  const { byGroup, ready } = buildActualQualifiersFromFotmobStandings(fotmobStandings, completed)
  ...
  const thirdAssignments = ready
    ? inferThirdPlaceAssignmentsFromR32(r32Api, groupMatches, byGroup)
    : {}
  const teamToGroup = buildTeamToGroupLetterMap(groupMatches)
  ...
  return roundOf32Map.map(slot => {
    ...
    return {
      ...
      home,
      away,
      homeCrest: crestLookup.crestFor(rawHome) ?? api?.homeCrest ?? null,
      awayCrest: crestLookup.crestFor(rawAway) ?? api?.awayCrest ?? null,
