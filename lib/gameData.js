...
export function calcParticipantScoreColumns(participant, group, scoringOpts = {}) {
  ...
  const inicioRaw =
    inicio.gep + inicio.resultado + inicio.advance
  const knockoutRaw =
  ...
  const gepPts = roundPts(inicio.gep * PHASE_WEIGHT.inicio + knockoutReal.gep * PHASE_WEIGHT.knockoutReal)
  const resultadoPts = roundPts(
    inicio.resultado * PHASE_WEIGHT.inicio + knockoutReal.resultado * PHASE_WEIGHT.knockoutReal,
  )
  const advancePts = roundPts(
    inicio.advance * PHASE_WEIGHT.inicio + knockoutReal.advance * PHASE_WEIGHT.knockoutReal,
  )
