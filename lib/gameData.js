...
import { migrateParticipantPredictions, migrateGroupResults } from './matchIdMap.js'
import { indexApiMatches } from './apiMatchScores.js'
import { lookupEliminatoriasKoPred } from './knockoutBridge.js'
import {
  buildInicioKnockoutScoringState,
  calcInicioKnockoutPointsForId,
} from './inicioKnockoutScoring.js'
import { bonusPlayerNamesMatch } from './bonusPlayerMatch.js'
...
/** Marcador exacto acertado (incluye comprobación de cruce en KO). */
export function isExactScoreHit(prediction, result, opts = {}) {
  if (!prediction || result?.home == null || result?.away == null) return false
  if (prediction.home == null || prediction.away == null) return false
  return calcMatchPointsSplit(prediction, result, opts).resultado > 0
}
...
export function calcBonusPointsSplit(predictions, actuals) {
  ...
  ESPECIAL_FIELDS.forEach(f => {
    if (predictions?.[f] && actuals?.[f] && bonusPlayerNamesMatch(predictions[f], actuals[f])) {
      especial += SCORING[f] || 0
    }
  })
  if (predictions?.mvp && actuals?.mvp && bonusPlayerNamesMatch(predictions.mvp, actuals.mvp)) {
    mvp += SCORING.mvp || 0
  }
  return { especial, mvp }
}
...
export function calcParticipantScoreColumns(participant, group, scoringOpts = {}) {
  ...
  const inicioKoState = buildInicioKnockoutScoringState(participant, {
    groupMatches,
    knockoutMatches,
    knockoutResults: results.knockout,
    groupResults: results.group,
    fotmobStandings: scoringOpts.fotmobStandings ?? null,
    apiMatches: scoringOpts.apiMatches ?? [],
  })
  const apiById = indexApiMatches(scoringOpts.apiMatches)
  ...
  Object.entries(preds.inicioKnockout || {}).forEach(([id, pred]) => {
    if (!pred) return
    addTo(inicio, calcInicioKnockoutPointsForId(id, pred, inicioKoState))
  })

  const elimScoringMatches = listEliminatoriasScoringMatches(knockoutMatches, preds.knockout, {
    groupMatches,
    apiMatches: scoringOpts.apiMatches ?? [],
    fotmobStandings: scoringOpts.fotmobStandings ?? null,
  })
  for (const m of elimScoringMatches) {
    const pred = lookupEliminatoriasKoPred(preds.knockout, m)
  ...
  const qualification = calcGroupQualificationPoints(participant, {
    ...scoringOpts,
    groupResults: results.group,
  })
