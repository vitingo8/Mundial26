...
import { calcKnockoutAdvanceBonus, enrichKnockoutResultWithAdvances } from './knockoutAdvances.js'
import {
...
export function calcParticipantScoreColumns(participant, group, scoringOpts = {}) {
  ...
  Object.entries(preds.knockout || {}).forEach(([id, pred]) => {
    ...
    const rawRes = resolveKnockoutResult(id, results.knockout, koCtx)
    if (!pred || rawRes?.home == null || rawRes?.away == null) return
    const res = enrichKnockoutResultWithAdvances(rawRes, apiById[String(id)])
    let { predictedTeams, actualTeams } = resolveKnockoutTeamsForScoring(id, res, koCtx)
