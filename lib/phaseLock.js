...
export function isPredPhaseEditable(predPhase, group, isAdmin, adminOverride) {
  if (isPhaseLocked(group?.phase, predPhase, isAdmin, adminOverride)) return false
  if (predPhase === 'group' && isDeadlinePassed(group?.group_deadline)) return false
  if (predPhase === 'knockout' && isDeadlinePassed(group?.knockout_deadline)) return false
  if (predPhase === 'bonuses' && isDeadlinePassed(group?.bonus_deadline)) return false
  return true
}
