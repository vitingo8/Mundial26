import { isGroupDeadlinePassed, isKnockoutPhaseFullyLocked } from './deadlines'
...
export function isPredPhaseEditable(predPhase, group, isAdmin, adminOverride, options = {}) {
  ...
  if (predPhase === 'knockout') {
    ...
    if (isKnockoutPhaseFullyLocked(knockoutMatches, group)) return false
    return true
  }
  if (predPhase === 'bonuses' && isGroupDeadlinePassed(group)) return false
  return true
}
