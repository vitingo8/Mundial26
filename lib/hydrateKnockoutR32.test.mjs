...
describe('buildLiveKnockoutMatches dedupe', () => {
  it('does not duplicate Colombia vs Ghana when API date mapping is wrong', () => {
    ...
    const groupMatches = catalogForLetters('ABCDEFGHIJKL')
    const apiMatches = [
      ...finishedGroupApiMatches('ABCDEFGHIJKL'),
      ...groupMatches
        .filter(m => m.group === 'K' || m.group === 'L')
        .map(m => ({
          id: m.id,
          status: 'FINISHED',
          score: { fullTime: { home: 1, away: 0 } },
    ...
    const live = buildLiveKnockoutMatches(
      knockoutMatches,
      fotmobStandings,
      groupMatches,
      apiMatches,
    )
    const colGha = live.filter(
      m => teamsMatch(m.home, 'Colombia') && teamsMatch(m.away, 'Ghana'),
    )
    assert.equal(colGha.length, 1)
    assert.equal(colGha[0].matchNumber, 87)
    assert.equal(colGha[0].id, 'api-col-gha')
  })
})
