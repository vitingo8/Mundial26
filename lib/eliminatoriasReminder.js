...
/** Texto breve de lo que falta en la predicción (para avisos). */
export function getEliminatoriasPredIncompleteReason(pred) {
  if (!pred || !isScoreFilled(pred.home) || !isScoreFilled(pred.away)) {
    return 'Falta el marcador'
  }
  if (needsKnockoutAdvancePick(pred) && !resolveKnockoutAdvanceSide(pred)) {
    return 'Falta indicar quién pasa (empate)'
  }
  return null
}
