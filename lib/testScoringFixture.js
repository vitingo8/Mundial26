...
import { calcParticipantScoreColumns, calcMatchPointsSplit } from './gameData.js'
...
export function calcInicioKnockoutFixturePts() {
  ...
  for (const [id, pred] of Object.entries(TEST_PREDICTIONS.inicioKnockout || {})) {
    const res = TEST_REAL_RESULTS.knockout?.[id]
    if (!res) continue
    const split = calcMatchPointsSplit(pred, res, { knockout: true })
