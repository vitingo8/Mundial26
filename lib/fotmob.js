import { buildFotmobLiveCommentary, buildFotmobLtcFeed, buildPlayerLookup } from './matchDetail.js'
...
async function enrichDetailWithLtc(detail, pageProps, matchId) {
  try {
    const ltc = await fetchFotmobLtc(pageProps, matchId)
    if (!ltc?.events?.length) return detail
    const lang = pickLtcLang(pageProps?.content?.liveticker?.langs)
    const pulseData = await fetchFotmobPulseBundle(matchId)
    const playerLookup = buildPlayerLookup(detail.homeTeam, detail.awayTeam)
    return {
      ...detail,
      liveCommentary: buildFotmobLtcFeed(ltc, pulseData, lang, { playerLookup }),
    }
  } catch {
    return detail
  }
...
export async function fetchFotmobMatchDetail(matchId, pageUrl) {
  try {
    ...
    if (api?.general?.matchId) {
      ...
      return enrichDetailWithLtc(detail, api, matchId)
    }
  } catch {
  ...
  if (!slug) {
    ...
    return transformFotmobListMatch(scoreOnly)
  }
  ...
  return enrichDetailWithLtc(detail, pageProps, matchId)
}

function teamBlockFromNames(homeName, awayName, homeId, awayId, homeScore, awayScore) {
  return {
    homeTeam: {
      id: homeId,
      name: homeName,
      shortName: homeName,
      crest: teamCrestUrl(homeId),
    },
    awayTeam: {
      id: awayId,
      name: awayName,
      shortName: awayName,
      crest: teamCrestUrl(awayId),
    },
    score: {
...
function mapFotmobEvent(ev, homeName, awayName) {
  ...
  if (ev.type === 'Goal') {
    return {
      ...
      scorer: { name: ev.player?.name || ev.nameStr },
      assist: ev.assistInput ? { name: ev.assistInput } : ev.assistStr ? { name: ev.assistStr.replace(/^assist by\s+/i, '') } : undefined,
      team: { name: teamName },
  ...
  if (ev.type === 'Card') {
    ...
    return {
      ...
      minute,
      card,
      player: { name: ev.player?.name || ev.nameStr },
      team: { name: teamName },
    }
  }
  if (ev.type === 'Substitution' || ev.type === 'subst') {
    return {
      kind: 'sub',
      minute,
      playerOut: { name: ev.playerOut?.name || ev.outName || ev.swap?.[0]?.name },
      playerIn: { name: ev.playerIn?.name || ev.inName || ev.swap?.[1]?.name },
      team: { name: teamName },
    }
  }
  return null
...
function mapLineupPlayers(players = []) {
  return players.map(p => ({
    ...
    layout: p.horizontalLayout
      ...
      : null,
    photoUrl: fotmobPlayerPhotoUrl(p.id),
  }))
}
