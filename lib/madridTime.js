...
// Construir Intl.DateTimeFormat es caro; se reutilizan instancias a nivel de módulo.
let wallClockFmt = null
let madridDisplayFmtCache = new Map()
let offsetFmt = null

function getWallClockFmt() {
  if (!wallClockFmt) {
    wallClockFmt = new Intl.DateTimeFormat('en-US', {
      timeZone: MADRID_TZ,
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
...
export function formatMadridDateTime(iso, options = {}) {
  ...
  let fmt = madridDisplayFmtCache.get(key)
  if (!fmt) {
    fmt = new Intl.DateTimeFormat('es-ES', {
      timeZone: MADRID_TZ,
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    ...
    madridDisplayFmtCache.set(key, fmt)
  }
  return fmt.format(d)
}
...
export function madridOffsetLabel(iso) {
  ...
  const parts = offsetFmt.formatToParts(d)
