...
export function getScoringDisputedLimits(group, scoringOpts = {}) {
  ...
  const qualificationRaw =
    (buildActualQualifiersFromApiR32(knockoutMatches, groupMatches).resolvedCount ?? 0) *
    (QUALIFICATION_SCORING.qualifies + QUALIFICATION_SCORING.exactPosition)
  ...
  return { total, inicioPts, knockoutPts, especialPts, mvpPts }
}
...
export function formatCategoryPct(value, total) {
  const v = Number(value) || 0
