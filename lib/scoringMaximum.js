...
export function countConfirmedQualificationSlots(scoringOpts = {}) {
  const { fotmobStandings, knockoutMatches, groupMatches, apiMatches = [] } = scoringOpts
  ...
  if (hasLiveMatches) return 0

  return buildActualQualifiersFromApiR32(knockoutMatches, groupMatches, completed).resolvedCount ?? 0
}
