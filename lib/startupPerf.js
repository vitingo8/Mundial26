 ...
 *
 * Dev: activos siempre.
 * Producción: ?perf=1 en la URL, o localStorage.setItem('porra_startup_perf', '1') y recarga.
...
function enabled() {
  ...
  try {
    if (localStorage.getItem('porra_startup_perf') === '1') return true
...
function pauseHint(delta, prevLabel, hiddenMs = 0) {
  if (delta < WARN_MS) return null
  const prev = prevLabel ? ` tras «${prevLabel}»` : ''
  if (hiddenMs >= WARN_MS) {
    return `Pestaña en segundo plano ~${(hiddenMs / 1000).toFixed(1)} s${prev} (pantalla apagada / cambio de app). No es un bloqueo real del hilo principal.`
  }
...
// Tiempo con la pestaña oculta (pantalla apagada, cambio de app, minimizado):
// los deltas entre marcas no reflejan bloqueo real del hilo principal en ese caso.
let hiddenAccumMs = 0
let hiddenSince = null
let lastMarkHiddenAccumMs = 0
...
if (typeof document !== 'undefined') {
  if (isHiddenNow()) hiddenSince = nowMs()
  document.addEventListener('visibilitychange', () => {
    if (isHiddenNow()) {
      hiddenSince = nowMs()
    } else if (hiddenSince != null) {
      hiddenAccumMs += nowMs() - hiddenSince
      hiddenSince = null
    }
  })
}

function hiddenMsUpTo(t) {
  return hiddenAccumMs + (hiddenSince != null ? Math.max(0, t - hiddenSince) : 0)
}
...
function logPauseHint(delta, prevLabel, hiddenMs) {
  const hint = pauseHint(delta, prevLabel, hiddenMs)
  if (!hint) return
  const fn = hiddenMs >= WARN_MS
    ? console.info
    : delta >= CRITICAL_MS ? console.error : delta >= SLOW_MS ? console.warn : console.info
...
export function perfMark(phase, message, extra) {
  ...
  const hiddenMs = hiddenMsUpTo(t) - lastMarkHiddenAccumMs
  const effectiveDelta = Math.max(0, delta - hiddenMs)
  const sev = severity(effectiveDelta)
  const entry = {
    phase,
    message,
    sinceNav: Math.round(sinceNav),
    delta: Math.round(delta),
    hidden_ms: hiddenMs >= 1 ? Math.round(hiddenMs) : undefined,
    severity: sev,
    extra: extra ?? null,
  }
  marks.push(entry)

  if (marks.length > 1) logPauseHint(effectiveDelta, lastLabel, hiddenMs)
  logLine(phase, message, extra, delta, sinceNav, sev)

  lastMark = t
  lastLabel = message
  lastMarkHiddenAccumMs = hiddenMsUpTo(t)
}
  ...
  try {
    ...
    const obs = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        ...
        } else if (!minimalWarned && entry.duration >= CRITICAL_MS) {
          minimalWarned = true
          console.warn(
            '[porra] Carga lenta: el hilo principal estuvo bloqueado ~',
            `${(entry.duration / 1000).toFixed(1)} s`,
            '(Chrome lo muestra como [Violation] message handler).',
            'Diagnóstico detallado: recarga con ?perf=1 en la URL',
            'o localStorage.setItem("porra_startup_perf","1") y filtra consola por "porra:arranque".',
          )
        }
      }
    })
...
export function perfSummary({ auto = false } = {}) {
  ...
  if (slow.length) {
    console.warn(`${LOG} Top pausas entre hitos (Δ ms; hidden_ms = pestaña oculta durante la pausa):`)
    console.table(slow.slice(0, 8).map(m => ({
      fase: PHASE[m.phase]?.label || m.phase,
      mensaje: m.message,
      delta_ms: m.delta,
      hidden_ms: m.hidden_ms ?? 0,
      t_ms: m.sinceNav,
      gravedad: m.severity,
    })))
  }
