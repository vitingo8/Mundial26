...
const PAUSE_SHORT_RE = /^(HT|MT|ET|FT|Descanso|Half|Break|Int|Pause|Interval)/i

/** Minuto jugable desde liveTime.short (null si es HT, MT, etc.). */
export function parseLiveMinuteShort(liveTime) {
  if (!liveTime?.short) return null
  const s = String(liveTime.short).trim()
  if (PAUSE_SHORT_RE.test(s)) return null
  const m = s.match(/^(\d+)/)
  return m ? Number(m[1]) : null
}

/** Partido en pausa (descanso, medio tiempo, entre periodos). El reloj no debe avanzar. */
export function isLiveClockPaused(status, liveTime) {
  if (status === 'PAUSED') return true
  if (!liveTime?.short) return false
  const s = String(liveTime.short).trim()
  if (PAUSE_SHORT_RE.test(s)) return true
  if (!/^\d/.test(s) && parseLiveMinuteShort(liveTime) == null) return true
  return false
}

export function buildLiveClockAnchor(liveTime, minute) {
  if (isLiveClockPaused(null, liveTime)) return null
  ...
  if (totalSeconds === 0 && liveTime?.short) {
    const parsedMin = parseLiveMinuteShort(liveTime)
    if (parsedMin != null) totalSeconds = parsedMin * 60
  }
...
export function formatPausedLiveClock() {
  return { clock: 'HT', addedTime: null, compact: 'HT', short: 'HT' }
}
