import { enrichMatch } from './fifaMatchNumbers.js'
...
/** Client: fetch via Next.js proxy (keeps API key on server) */
...
export async function fetchWcMatchesClient() {
  const data = await fetchWcResource('matches')
  return data.matches || []
}

export async function fetchWcTeamsClient() {
  const data = await fetchWcResource('teams')
  return data.teams || []
}

export async function fetchWcMatchClient(matchId) {
  return fetchWcResource('match', { id: String(matchId) })
}

/** Server: direct call to football-data.org */
export async function footballDataFetch(path, searchParams = {}) {
