...
const scoringOpts = buildScoringOptsFromWc(wc)
const { groupMatches: gm, knockoutMatches: km } = scoringOpts
const kmBroken = km.map(m => m.id === '4653842' ? { ...m, matchNumber: undefined, fifaMatchLabel: null } : m)

for (const u of users || []) {
  const now = calcParticipantScoreColumns({ predictions: u.predictions }, group, scoringOpts)
  const before = calcParticipantScoreColumns({ predictions: u.predictions }, group, { ...scoringOpts, knockoutMatches: kmBroken })
